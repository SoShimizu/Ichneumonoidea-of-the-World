...
export default function TaxonomicTree({
  ...
  const getSpecialChildren = useCallback((nodeId) => {
   ...
   const getAuthorsText = useCallback((scientificNameId) => {
    const authors = scientificNameAuthors
      .filter(rel => rel.scientific_name_id === scientificNameId).sort((a, b) => a.author_order - b.author_order)
      .map(rel => rel.author?.last_name_eng).filter(Boolean);
    if (!authors.length) return ""; if (authors.length === 1) return authors[0]; if (authors.length === 2) return `${authors[0]} & ${authors[1]}`;
    return `${authors.slice(0, -1).join(", ")} & ${authors.at(-1)}`;
   }, [scientificNameAuthors]);

   // 子孫カウント (ベース参照コードに合わせる - 有効な子のみカウント)
   const getDescendantCountByRank = useCallback((node, visited = new Set()) => {
       if (!node || !node.id || visited.has(node.id)) return {};
       visited.add(node.id);
       const counts = {};
       // 【注意】ベース参照コードに合わせ、getValidChildren を使う
       ...
       return counts;
   }, [getValidChildren]); // getValidChildren に依存

   // 子孫情報ライン (ベース参照コードに合わせる)
  ...
  useEffect(() => {
    ...
    // 【修正】クリック動作変更のため、初回展開のみに限定しない方が良いかも？
    // setExpandedNodes(newExpanded); // ← 初回のみならこれ
    setExpandedNodes(prev => ({ ...prev, ...newExpanded })); // マージする方が自然か？ 要件確認
  ...
  }, [getParentChain, expandedNodes, setExpandedNodes]);
    ...
    return (
      <Box sx={{ position: 'relative', ml: level * indentMultiplier }}>
        ...
        <Stack direction="row" alignItems="center" spacing={0.5} sx={{ bgcolor: isTarget ? "rgba(127,255,212,0.15)" : "transparent", borderRadius: 1, p: 0.3, my: 0.2, minHeight: '24px' }}>
            ...
            {/* 【修正】クリックエリア + 子孫情報 + 詳細ボタン */}
            <Box flexGrow={1} onClick={() => canExpand && handleNodeClick(node)} sx={{ cursor: canExpand ? 'pointer' : 'default', display: 'flex', alignItems: 'baseline', flexWrap: 'nowrap' }}> {/* nowrap追加 */}
              {/* 学名部分 (クリック可能) */}
              <Typography variant="body2" component="span" sx={{ display: 'inline', whiteSpace: 'nowrap' }}> {/* nowrap追加 */}
                {/* Rank */}
              ...
              {/* 子孫情報 (including...) - 学名の隣 */}
              {descendantLine && (
                  <Typography component="span" variant="caption" sx={{ ml: 1, color: "#888", whiteSpace: 'nowrap', flexShrink: 0 }}> {/* flexShrink追加 */}
                      {descendantLine}
                  </Typography>
            )}
...
{onSelectScientificName && (
  <Tooltip title="View Details" arrow>
      ...
      >
        <Button
          ...
          sx={{
            ...
            "&:hover": { bgcolor: "rgba(127,255,212,0.1)" }
          }}
        >
          ...
          <Typography variant="caption" sx={{ display: { xs: 'none', sm: 'inline' } }}>
            ...
            */}
              ...
              {onSelectScientificName && (
                <Tooltip title="View Details" arrow>
                  <Box component="span" sx={{ ml: 1, display: 'inline-flex', alignItems: 'center', flexShrink: 0 }} onClick={(e) => e.stopPropagation()}> {/* flexShrink追加 */}
                      ...
                      >
                        <InfoOutlinedIcon sx={{ fontSize: '1rem', mr: { xs: 0, sm: 0.3 } }} /> {/* モバイルではアイコンのみ */}
    ...
    getDescendantCountByRank, getDescendantLine, // 子孫カウント関数を追加
    scientificNameAuthors, allScientificNames, setExpandedNodes
  ]);
